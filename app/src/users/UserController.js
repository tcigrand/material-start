(function() {
  ...
  angular
    ...
    .controller('UserController', ['$scope',
      'userService', '$mdSidenav', '$mdBottomSheet', '$log', '$q', '$router',
      UserController
    ]).controller('ShotchartController', [ShotchartController])
    .directive('shotchart', function() {
      var data = [{
        "x": 2,
        "y": 9,
        "made": 3,
      ...
      d3.select(document.getElementById('chart'))
        ...
        .chart("BasketballShotChart", {
          ...
          threePointSideRadius: 20.75,
          threePointCutoffLength: 5,
          keyWidth: 12,
          restrictedCircleRadius: 3,
      ...
      $('.shot-chart-hexagon').tipsy({
        ...
        title: function() {
          ...
          if (d.length == 1)
            ...
            numberMade = d.reduce(function(a, b) {
              return a.made + b.made
            });

          return '<div>' +
            '<span style = "font-size:18px;color:' + color + '">Summary</span>' +
            '<br>Made: ' + numberMade +
            '<br>Attempts: ' + d.attempts +
            '<br>%: ' + ((numberMade / d.attempts) * 100).toFixed(2) +
            '</div>';
        }
      });
    return {};
   ...
   * @constructor
   */
  function UserController($scope,userService, $mdSidenav, $mdBottomSheet, $log, $q, $router) {
    var self = this;
    ...
    userService
      .loadAllUsers()
      .then(function(users) {
        self.users = [].concat(users).map(function(user) {
          return new User(user);
        });
        self.homePlayers = self.users.filter(function(user) {
          return user.team === 'home';
        });
        self.awayPlayers = self.users.filter(function(user) {
          return user.team === 'away';
        });
  ...
  function User(user) {
    ...
    this.statLine = '';
    ['points', 'rebounds', 'assists'].forEach(function(key, i) {
      if (i != 0)
        this.statLine += ', '
      this.statLine += this[key] + ' ' + key;
    }.bind(this));
  }
